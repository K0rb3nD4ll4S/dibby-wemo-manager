...
- **Schedule-rule enforcement window** *(refined in v2.0.39)* — for **5 minutes** after a Schedule rule fires, the scheduler re-asserts the intended state on every 10-second poll. Catches silent SOAP failures, confirm-read races, and someone-flips-it-immediately-after toggles. **After the 5-minute window expires, the enforcement stops** — a human turning the light back on at 11 pm has clearly chosen to override the 10:30 pm OFF schedule, and the scheduler stops fighting them. Countdown / Away / Trigger / AlwaysOn rules are intentionally exempt — they have their own state semantics. Devices with no Schedule entry today are not enforced — manual toggles work normally there.
